...
1. うら側から、`長いスペーサー`を`短いネジ（3mm）`でとめる。（4箇所）
...
1. おもて側から、`短いスペーサー`を`短いネジ（2mm）`でとめる（11箇所）
...
1. `すべてのスイッチプレート`の裏面に`太くて薄いゴムシート`を貼る
    ...
    **☠️危険：異なる厚さのゴムシートを貼ると、正しく組み立てられません。**
    ...
    〓すべてのプレートの写真を載せ、「このように貼る」とする（DN001x系のみ。以降はプレート裏面に貼り付け位置をシルクで示す予定。）

### 基板とスイッチプレートをひとつに組み合わせ、スイッチを取り付ける
...
1. `基板Top`と`基板Mid中央`を組み合わせる
    ...
    `基板Top`のおもて側から、`基板Mid中央`を`短いネジ（3mm）`で止めます（6箇所）
