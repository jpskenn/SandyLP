...
1. キーボードのレイアウトの右側の一番下にある`…`をクリックして、表示されたメニューから`Test Matrix Mode`を選択。
...
1. 【該当者のみ】うら側から`LED`を取り付ける（左右に3個。計6個）
    ...
    （〓ファームの設定でRGB LightingをOFFにしとかんか？→ケーブル接続時に、一瞬だけ赤色点灯する場合があります）
...
〓Vialに限定せず、ファームウェアの書き換えについて書く。（BOOTSELのボタンとか）
