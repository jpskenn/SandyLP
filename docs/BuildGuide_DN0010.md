...
### Base用スイッチプレートの捨て基板を切り離す
  ...
  `Base用スイッチプレート`上部の捨て基板（プレート外側の枠）を、ミシン目に沿って折り取るようにして切り離します。  
  ...
  ![切り離し箇所](/assets/BuildGuide_DN0010/IMG_5233.jpeg)
...
1. `ダイオード`を取り付ける（6個）
    ...
    基板に描かれた枠のうち、短辺が閉じている方向（▲マークが示す先の少し太い線がある方向）にダイオードの印を合わせて置きます。  
    予備はんだしたところへ、ダイオードの端子をはんだ付けします。  
    最後に、もう一方の端子をはんだ付けします。

    ![ダイオード取り付け位置](../assets/BuildGuide_DN0010/socket_direction.png)
    ![ダイオードを取り付けた状態](../assets/BuildGuide_DN0010/socket_direction.png)
...
1. うら側から、`長いスペーサー`を`短いネジ`でとめる。（4箇所）
...
1. `基板Mid左右`2枚のうち、ひとつを「R Back」と書かれた面が見えるように置き、もうひとつを「L Back」と書かれた面が見えるように置く。

1. `ダイオード`を取り付ける（それぞれ2個、計4個）

1. `Chocスイッチ用ソケット`を取り付ける（それぞれ2個、計4個）

1. おもて側から、`ワッシャー`を挟んだ`ナット`を`長いネジ`でとめる。（それぞれ4箇所、計8箇所）
...
1. `基板Base`と`基板Mid中央`を組み合わせる
    ...
    `基板Mid`のおもて側から、`長いスペーサー`を取り付けます。（2箇所）
...
1. `基板Top`と`基板Mid中央`を組み合わせる
    ...
    `基板Top`のおもて側から、`基板Mid`を`短いネジ`で止めます（6箇所）
...
1. `すべてのスイッチプレート`の裏面に`太くて薄いゴムシート`を貼る
...
1. `基板Mid中央`に、`Mid中央用スイッチプレート`と`スイッチ`を取り付ける
...
1. [Remap](https://remap-keys.app/)の`Test Matrix Mode`を使用して、すべてのキースイッチが正常に反応するか確認する
    ...
    〓TODO：確認方法をどうするか検討中。初期ファームウェアをVialにするとマトリクステストでセキュアロック解除が必要になり、スイッチつけてないので無理。となると、VIAファームを入れといてRemapでテストか？
...
- レイヤー構成とレイヤーインジケータLEDの色  
  ...
  LEDは左右とも同じパターンで点灯します。`－`は元のレイヤー色、`×`は消灯を意味します。
    ...
    | :-: | :-- | :-- | :-: |
    | 0 | ベースレイヤー #1 | SemiErgo 英語配列 | 白, 白, 白 |
    | 1 | ベースレイヤー #2 | 一般的なロースタッガード 英語配列 | 青, 青, 青 |
