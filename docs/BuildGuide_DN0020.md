# SandyLP（DN0020）ビルドガイド

このドキュメントは、SandyLP（DN0020）を組み立てるためのビルドガイドです。

開発番号の`DN0020`は、各基板のうら側に記載されています。  
（Mid左右用スイッチプレートには記載がありません。）
![開発ナンバー](../assets/BuildGuide_DN0020/icon_develop_number.png)
...
- [SandyLP（DN0020）ビルドガイド](#sandylpdn0020ビルドガイド)
  - [目次](#目次)
  - [注意事項](#注意事項)
  - [部品リスト](#部品リスト)
    - [キット内容](#キット内容)
...
〓TODO
〓 DN0020は～というエクスキューズ書くの？
〓 基本的にクリティカルな何かだけ書くようにする。
...
![基板](../assets/BuildGuide_DN0020/parts_pcbs.jpeg)  

プレート類  
左上から時計回りに、Top用スイッチプレート、Mid中央用スイッチプレート、Mid左右用スイッチプレート（2枚）、Base用スイッチプレート、ボトムプレート。  
![プレート類](../assets/BuildGuide_DN0020/parts_plates.jpeg)  

その他部品  
![その他部品](../assets/BuildGuide_DN0020/〓)  
...
- おもて側とうら側を間違えないよう、基板うら側の「Back Side」の表記を確認しながら作業を進めます。
  ...
  ![Back Sideの表記](../assets/BuildGuide_DN0020/icon_back-side.png)
...
1. キースイッチの反応を確認する。
    ...
    ![スイッチ取り付け用パッド](../assets/BuildGuide_DN0020/icon_pad.png)

    すべてのパッドで実施した後、以下のように色が変われば正常です。  
    ![基板Base確認結果](../assets/BuildGuide_DN0020/remap_base_test_complete.png)
...
1. `ダイオード`を取り付ける。（6個）
    ...
    ![ダイオードの方向](../assets/BuildGuide_DN0020/icon_diode_direction.png)
    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_top_back_diode.png)
...
1. `Chocスイッチ用ソケット`を取り付ける。（6個）
    ...
    ![ダイオードの方向](../assets/BuildGuide_DN0020/icon_socket_direction.png)

    はんだごての温度を270度程度にセットして、はんだ付けします。  

    ソケットが基板から浮かないように注意し、基板にぴったりと接するように取り付けます。

    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_top_back_socket.png)
...
1. `ダイオード`を取り付ける。（7個）
    ...
    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_mid_back_diode.png)

1. `Chocスイッチ用ソケット`を取り付ける。（7個）

    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_mid_back_socket.png)
...
1. `ダイオード`を取り付ける。（左右それぞれ2個、計4個）
    ...
    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_mid-iso_L_back_diode.png)

    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_mid-iso_R_back_diode.png)

1. `Chocスイッチ用ソケット`を取り付ける。（左右それぞれ2個、計4個）

    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_mid-iso_L_back_socket.png)

    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_mid-iso_R_back_socket.png)
...
1. うら側から、`Chocスイッチ用ソケット`を取り付ける。（33個。ロータリーエンコーダを取り付ける場合は32個）
    ...
    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_base_back_socket.png)
...
1. 【該当者のみ】うら側から`LED`を取り付ける。（左右に3個ずつ。計6個）
    ...
    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_base_back_led.png)

    LED取り付け用パッド（1個のLEDに4個のパッド）のひとつに、予備はんだしておきます。  
    基板に描かれた印にLEDの端子の切り欠き部分を合わせ、LED発光部が基板おもて側になるように置きます。  

    ![LED取り付けマーキング](../assets/BuildGuide_DN0020/icon_led_direction.png)
...
1. 【該当者のみ】うら側から`スピーカー`を取り付ける。
    ...
    ![LED取り付けマーキング](../assets/BuildGuide_DN0020/pcb_base_back_speaker.png)
...
1. 【該当者のみ】おもて側から`ロータリーエンコーダ`を取り付ける。（1個）
    ...
    **☠️危険：ロータリーエンコーダは「おもて側」からはめ込みます。**
    ...
    ![取り付け位置](../assets/BuildGuide_DN0020/pcb_base_front_encoder.png)
...
![ネジ穴の印](../assets/BuildGuide_DN0020/icon_screw.png)  
...
1. `基板Top`うら側から、`小さな薄いゴムシート`を貼る。（6個）
    ...
    貼り付け例  
    ![貼り付け位置](../assets/BuildGuide_DN0020/pcb_top_back_form.jpeg)
...
1. `基板Mid中央`のうら側から、`ワッシャー`を挟んだ`スペーサー`を`短いネジ（3mm）`でとめる。（4箇所）
    ...
    ![基板Mid中央 うら側ネジ止め箇所](../assets/BuildGuide_DN0020/pcb_mid_back_screw.png)
...
1. おもて側から、`ワッシャー`を挟んだ`ナット`を`長いネジ`でとめる。（8箇所）
    ...
    ![基板Mid中央 おもて側ネジ止め箇所](../assets/BuildGuide_DN0020/pcb_mid_front_screw.png)
...
1. うら側の空いたスペースに`細くて薄いゴムシート`を貼る。
    ...
    **☠️異なる厚さのゴムシートを貼ると、正しく組み立てられません。**
    ...
    ゴムシート貼り付け位置  
    ![貼り付け位置](../assets/BuildGuide_DN0020/pcb_mid_back_form.jpeg)
...
1. `基板Mid左右`のおもて側から、`ワッシャー`を挟んだ`ナット`を`長いネジ`でとめる。（左右それぞれ4箇所、計8箇所）
    ...
    **☠️危険：中央部分のネジ穴と隣のソケット用パッドが近いので、ワッシャーをなるべくパッドから離すような気持ちで取り付ける。**
    ...
    ![基板Mid左右 おもて側ネジ止め箇所](../assets/BuildGuide_DN0020/pcb_mid-iso_R_back_screw.png)

    ![基板Mid左右 おもて側ネジ止め箇所](../assets/BuildGuide_DN0020/pcb_mid-iso_L_back_screw.png)
...
1. うら側の空いたスペースに`細くて薄いゴムシート`を貼る。
    ...
    **☠️危険：異なる厚さのゴムシートを貼ると、正しく組み立てられません。**
    ...
    ゴムシート貼り付け位置  
    ![貼り付け位置](../assets/BuildGuide_DN0020/pcb_mid-iso_back_form.jpeg)
...
1. うら側から、`ワッシャー`を挟んだ`ナット`を`長いネジ`でとめる。（2箇所）
    ...
    ![基板Baseうら側ネジ止め箇所（長いネジ）](../assets/BuildGuide_DN0020/pcb_base_back_screw.png)
...
1. うら側から、`細くて分厚いゴムシート`を貼る。
    ...
    **☠️危険：異なる厚さのゴムシートを貼ると、正しく組み立てられません。**
    ...
    ゴムシート貼り付け位置  
    ![貼り付け位置](../assets/BuildGuide_DN0020/pcb_base_back_form.jpeg)
...
ゴムシート貼り付け位置  
Base用スイッチプレート  
![貼り付け位置](../assets/BuildGuide_DN0020/sw-plate_base_back_form.jpeg)

Mid中央用スイッチプレート  
![貼り付け位置](../assets/BuildGuide_DN0020/sw-plate_mid_back_form.jpeg)

Mid左右用スイッチプレート  
![貼り付け位置](../assets/BuildGuide_DN0020/sw-plate_mid-iso_back_form.jpeg)

Top用スイッチプレート  
![貼り付け位置](../assets/BuildGuide_DN0020/sw-plate_top_back_form.jpeg)
...
1. `基板Base`と`基板Mid中央`を組み合わせる。
    ...
    ![基板Mid中央 おもて側スペーサー取り付け箇所](../assets/BuildGuide_DN0020/pcb_mid_front_spacer.png)
...
1. `基板Base`と`基板Mid左右`を組み合わせる。
    ...
    **☠️危険：1箇所、ナット付近に小さなパーツが配置されているところがあります。レンチ等が小さなパーツに当たらないようにします。**
    ...
    ![基板Bass うら側ナット止め箇所](../assets/BuildGuide_DN0020/pcb_base_back_nut_to_mid-iso.png)
...
1. `Mid中央用スイッチプレート`とスイッチを取り付ける。
    ...
    ![スイッチがプレートから浮いている不正な状態](../assets/BuildGuide_DN0020/switch_install_fail.jpeg)
...
1. `ボトムプレート`うら側から、`短いネジ（3mm）`で止める。（11箇所）
    ...
    ![ボトムプレートうら側からのネジ止め箇所](../assets/BuildGuide_DN0020/plate_bottom_back_screw.png)
...
以上で組み立て完了です。
![完成](../assets/BuildGuide_DN0020/finish.jpeg)
...
- 基板等の設計ファイルやドキュメント、Remapの定義ファイルなど
    ...
    [SandyLPリポジトリ](https://github.com/jpskenn/SandyLP)のリリース[DN0020](https://github.com/jpskenn/SandyLP/releases/tag/DN0020-001)にまとめてあります。
...
1. 以下の操作をおこない、SandyLPをマスストレージデバイスとしてPCやMacに認識させます。
    ...
    ![BOOTSELボタン](../assets/BuildGuide_DN0020/icon_bootsel.jpeg)
...
![キーキャップガイド](../assets/BuildGuide_DN0020/keycaps_guide.png)
